...
const ExplorerItem: FC<explorerItemProps> = (props) => {
    ...
    const approveChanges = () => {
        if (props.type === 'list') {
            if (inputValue !== props.name) {
                axios.put(`/api/list/${props.id}`, { name: inputValue }).then(() => {
                    props.callback();
                });
            } else {
                cancelChangingName();
            ...
            if (inputValue !== props.name) {
                axios.put(`/api/folder/${props.id}`, { name: inputValue }).then(() => {
                    props.callback();
                });
            } else {
                cancelChangingName();
            ...
            axios.post(`/api/folder/${props.folderId}/add/folder`, { name: inputValue }).then(() => {
                props.callback();
            });
        } else if (props.type === 'add-list') {
            axios.post(`/api/folder/${props.folderId}/add/list`, { name: inputValue }).then(() => {
                props.callback();
            });
        }
    }
    ...
    const removeItem = () => {
        setShowMenu(false);
        axios.delete(`/api/${props.type}/${props.id}/`).then(() => {
            props.callback();
        });
    }
